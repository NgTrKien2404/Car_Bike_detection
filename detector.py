...
import os
# Load YOLO model
model = YOLO("model/best (2).pt")  # Thay đường dẫn model của bạn
...
def detect_webcam(root):
    ...
    def update_webcam():
        ...
        if is_running:
            webcam_window.after(30, update_webcam)
        else:
            cap.release()
            output_filename = get_next_filename("out_put/output_webcam", ".jpg")
            cv2.imwrite(output_filename, annotated)
